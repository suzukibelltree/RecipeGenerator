...
@Composable
fun MediumCategoryRanking(
    selectedCategoryName: String,
    parentCategoryId: String,
    grandparentParentCategoryId: String,
    ...
    if (viewModel.smallList.isEmpty() || viewModel.mediumRankingList.isEmpty()) {
        ...
        LazyColumn {
            ...
            val matchedSmallList = viewModel.smallList.filter { it.parentCategoryId == parentCategoryId }
                .take(if (viewModel.showMoreCategory.value) Int.MAX_VALUE else 4)

            items(matchedSmallList.chunked(2)) { rowItems ->
                Row(
                    modifier = Modifier.padding(8.dp),
                    horizontalArrangement = Arrangement.End
                ) {
                    rowItems.forEachIndexed { index, small ->
                        SmallCategoryCard(
                            small,
                            Modifier
            ...
            items(viewModel.mediumRankingList) { recipe ->
                Card(
                    onClick = {
